...
with open('02_27-51b.txt', 'r') as file:
    for s in file:
...
while data:
    kl = [data.pop()]
    for point in kl:
        sosed = [p for p in data if dist(point, p) < 1.1]
        for p1 in sosed:
            kl.append(p1)
            data.remove(p1)
    clusters.append(kl)
...
Px = int(mean([x for x, y in centroids]) * 10_0000)
Py = int(mean([y for x, y in centroids]) * 10_0000)
