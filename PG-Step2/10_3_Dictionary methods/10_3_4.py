...
res = {}
for word in set(lst):
    if lst.count(word) not in res:
        res.setdefault(lst.count(word), word)
    else:
        if word < res.get(lst.count(word)):
            res[lst.count(word)] = word
print(res[max(res)])
...
# d = {s.count(i): i for i in sorted(set(s), reverse=True)}
# print(d[max(d)])