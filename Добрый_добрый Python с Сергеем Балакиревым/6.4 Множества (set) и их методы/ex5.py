# Подвиг 7. В аккаунте YouTube Сергея прокомментировали очередное видео. Некоторые посетители оставляли несколько комментариев. Требуется по списку комментариев определить уникальное число комментаторов (полагается, что имена у разных комментаторов не совпадают). Комментарии поступают на вход программе в формате:
#
# имя 1: комментарий 1
# имя 2: комментарий 2
# ...
...
import sys
lst_in = list(map(str.strip, sys.stdin.readlines()))
lst_in = [item.split(sep = ':') for item in lst_in]
d = {key: value.lstrip() for key, value in lst_in}
print(len(set(d.keys())))
# print(len(set(line.split(':')[0] for line in open(0))))
