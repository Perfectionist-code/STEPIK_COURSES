...
book = [t[i](x) if t[i] != str else x.strip() for i, x in enumerate(input().split(","))]
match book:
    case [int(_), author, title] if len(author) > 5 and len(title) > 9:
        print('YES'.title())
    case [int(_), author, title, float(price)] if len(author) > 5 and price > 0:
        print('YES'.title())
    case [int(_), author, title, int(year)] if year > 2019:
        print('YES'.title())
    case [int(_), author, title, float(price), int(year)] if year > 2019 and price > 0:
        print('YES'.title())
    case _:
        print('NO'.title())
